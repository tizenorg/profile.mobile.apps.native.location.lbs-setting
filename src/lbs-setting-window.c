...
Evas_Object *create_win(const char *name)
{
	...
	if (eo) {
		...
		elm_win_wm_desktop_layout_support_set(eo, EINA_TRUE);
...
static void __setting_location_popup_agree_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (ad->ask_check) {
		...
		if (ischoose) {
			int ret = vconf_set_int(VCONF_LBS_SETTING_IS_SHOW_GPS_POPUP, KEY_DISABLED);
			LS_LOGE("ret:%d", ret);
		}
	}
...
static void __setting_location_myloc_set_key(void *data)
{
	...
	if (ad->is_myloc) { /* myloc on, show gps popup at the same time */
		...
		int ret = vconf_get_int(VCONF_LBS_SETTING_IS_SHOW_GPS_POPUP, &isShow);
		if (ret != 0) {
